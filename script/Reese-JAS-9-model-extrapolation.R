...
total.extrapolated.households <- matrix(NA,nrow=8,ncol=ncol(small.sites[,9:ncol(small.sites)]))
...
for(years in 9:ncol(small.sites)) {
  ...
  # If households are predicted to be occupied in the given year, continue prediction
    ...
    layer.7 <- raster::stackApply(temperature[[start:end]],indices=raster::nlayers(temperature[[start:end]]),fun='mean')
    # Prediction layer: Average annual precipitation
    layer.8 <- raster::stackApply(precipitation[[start:end]],indices=raster::nlayers(precipitation[[start:end]]),fun='mean')
    # Prediction layer: Average annual maize growing niche
    layer.9 <- raster::stackApply(growing.niche[[start:end]],indices=raster::nlayers(growing.niche[[start:end]]),fun='mean')
    ...
    extrapolated.households <- foreach::foreach(pitstructures = 1:8,.combine=rbind) %dopar% {
      ...
      # If households with given number of pitstructures are predicted to be occupied in the given year, continue prediction
        ...
        # Determine ratio of knowingly occupied cells within surveyed area with total surveyed cells with predictive raster values within one standard deviation of the average predictive values of knowingly occupied raster cells
...
vepii.households <- vepii.database[which(vepii.database$Center == 0 & vepii.database$TOTPITST < 9 & vepii.database$TOTPITST > 0 & vepii.database$recordtypedesc == 'Habitation' ),]
...
for(c in 48:61) {
  ...
  cc.sites <- rbind(vepii.database[which(vepii.database$TOTPITST >= 9 & vepii.database[,c] >= 1 ),],vepii.database[which(vepii.database$TOTPITST > 0 & vepii.database$TOTPITST < 9 & vepii.database$Center == 1 & vepii.database[,c] >= 1 ),])
  cc.households <- sum(cc.sites[,c])
