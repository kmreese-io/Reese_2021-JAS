...
sum.occupation.household <- as.numeric(colSums(occupation.household[,9:ncol(occupation.household)]))

## Determine total number of households with no ceramic information whatsoever
ss.unknown.sites <- utils::read.csv('/Users/kmreese/Documents/PROJECTS/CURRENT/Reese-JAS/output/results/occupation-unknowns.csv')
ss.unknown.sites$PITSTRUCTURES[is.na(ss.unknown.sites$PITSTRUCTURES)] <- 0
ss.unknown.sites <- ss.unknown.sites[which(ss.unknown.sites$PRIMARY == 'HABITATION' | ss.unknown.sites$PRIMARY == 'Habitation' | ss.unknown.sites$PRIMARY == 'GREATHOUSE' | ss.unknown.sites$PRIMARY == 'BIGHOUSE'),]
ss.unknown.households <- sum(ss.unknown.sites$PITSTRUCTURES)

# Calculate distribution of known households though time
sum.occupation.household.proportion <- sum.occupation.household / sum(as.numeric(sum.occupation.household))

# Multiply unknown households by known households distribution through time
unknown.household.distribution <- round(ss.unknown.households * sum.occupation.household.proportion)

# Import estimated numbers of extrapolated households by year
...
# Combine known households, no ceramic households, and extrapolated households for total regional population estimate
region.population.household <- sum.occupation.household + sum.extrapolated.households + unknown.household.distribution
...
utils::write.csv(vepii.calculated.population,'/Users/kmreese/Documents/PROJECTS/CURRENT/Reese-JAS/output/table-products/vepii-reconstructed.csv')
